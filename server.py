...
SCENES_DIR = ROOT / "scenes"          # 전시 전용 도입부 이미지 (이미지에 동봉)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ("/", "/write", "/write.html"):
            return self._file(ROOT / "write.html", "text/html; charset=utf-8")
        if p in ("/stage", "/stage.html"):
            return self._file(ROOT / "stage.html", "text/html; charset=utf-8")
        if p == "/api/round":
